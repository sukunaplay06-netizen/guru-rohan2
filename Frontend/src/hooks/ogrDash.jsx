...
export default function AdminDashboard() {
  const [loading, setLoading] = useState({ stats: true, sales: true, affiliates: true, courses: true, contacts: true, visitors: true });
  ...
  const fetchDashboard = async () => {
    ...
    try {
      ...
      const [statsRes, salesRes, affiliatesRes, coursesRes, contactsRes, visitorsRes] = await Promise.all([
        axios.get("/admin/dashboard"),
        axios.get("/admin/sales"),
        axios.get("/admin/affiliates"),
        axios.get("/admin/courses"),
        axios.get("/admin/contacts"),
        axios.get("/admin/dashboard/visitors")
      ]);
      ...
      setVisitorStats(visitorsRes.data || visitorStats);
    } catch (err) {
      console.error("[AdminDashboard] Error loading dashboard data:", err.response?.data || err.message);
      setError(err.response?.data?.message || "Failed to load dashboard data");
    } finally {
      setLoading({ stats: false, sales: false, affiliates: false, courses: false, contacts: false, visitors: false });
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">   {/* 5 columns bana diya */}

        {/* Existing cards: Total Sales, Commission, Platform Earnings, Affiliates */}

        <Card>
          <CardContent className="p-4">
            <p className="text-sm">Total Visits</p>
            <p className="text-xl font-bold">{visitorStats.totalVisits.toLocaleString()}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-xl font-bold">{visitorStats.uniqueVisitors.toLocaleString()}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-xl font-bold">{visitorStats.todayVisits.toLocaleString()}</p>
          </CardContent>
        </Card>

        {/* Already online users hai to usko bhi dikha sakte ho */}
        <Card>
          <CardContent className="p-4">
            <p className="text-sm flex items-center gap-2">
              <Users className="h-4 w-4 text-green-600" /> Online Now
            </p>
        ...
        <Card>
          <CardContent className="p-4">
            <p className="text-sm">Today's Unique</p>
            {loading.visitors ? (
              <p className="text-xl font-bold text-gray-400 animate-pulse">...</p>
            ) : (
              <p className="text-xl font-bold">{visitorStats.todayUniqueVisitors.toLocaleString()}</p>
            )}
          </CardContent>
        </Card>
